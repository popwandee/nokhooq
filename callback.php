...
foreach ($events as $event) {
  ...
  // แปลงข้อมูลจาก Postback Data เป็น array
        parse_str($event->getPostbackData(),$dataPostback);
        // ดึงค่า params กรณีมีค่า params
        $paramPostback = $event->getPostbackParams();
  ...
  if ($event instanceof \LINE\LINEBot\Event\MessageEvent\TextMessage) {
        ...
        $userId = $event->getUserId();// ดึงค่า userId มาไว้ใช้งาน
    ...
    // ถ้าเป็นข้อความ
    if($typeMessage=='text'){
        $userMessage = $event->getText(); // เก็บค่าข้อความที่ผู้ใช้พิมพ์
...
switch ($explodeText[0]) {
     ...
     case '#j':       
        ...
        if ($response->isSucceeded()) {
            ...
            $textReplyMessage = 'สวัสดีครับ คุณ '.$userData['displayName'].' ID '.$userData['userId'].$userData['statusMessage'].$userData['pictureUrl'];     
            }else{
      ...
      break;
	    ...
	    $textReplyMessage = "ไม่เข้าเงื่อนไขใดๆ เลย";
    $replyData = new TextMessageBuilder($textReplyMessage); 

	    
		break;
            }//end switch
    }//end if event = text Message
}// end foreach event
...
if($event->isRoomEvent()){
$response = $bot->getRoomMemberProfile($roomId, $userId);
 }else{ $response = $bot->getProfile($userId);}
if ($response->isSucceeded()) {
    $userData = $response->getJSONDecodedBody(); // return array     
    // $userData['userId']
    // $userData['displayName']
